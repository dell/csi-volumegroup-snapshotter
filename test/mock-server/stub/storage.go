...
func contains(expect, actual map[string]interface{}) bool {
	for key, val := range expect {
		...
		kind := reflect.TypeOf(actualValue)
		dataType := fmt.Sprint(kind)
		compare := strings.Contains(dataType, "string")
		// we will do deep compare only for non basic types
		if !compare {
			...
			strVal, _ := val.(string)
			strActual, _ := actualValue.(string)
			return strings.Contains(strActual, strVal)
		}
	}
	return true
}
...
func matches(expect, actual map[string]interface{}) bool {
	for keyExpect, valueExpect := range expect {
		...
		if err != nil {
			fmt.Printf("Error on matching regex %s with %s error:%v\n", valueExpectString, actualvalue, err)
		}
...
func readStubFromDir(path string) {
	...
	for _, file := range files {
		f := filepath.Clean(path + "/" + file.Name())
		byt, err := ioutil.ReadFile(f)
		if err != nil {
			fmt.Printf("Error when reading file %s. %v. skipping...", file.Name(), err)
			continue
		}
		...
		err = storeStub(stub)
		if err != nil {
			fmt.Printf("store stub error: %v", err)
		}
	}
}
